...
for index in indexsplit[rank]:
    ...
    print('Time taken for index %d = '%index, time()-begin)

comm.barrier()
if rank == 0:
    cl = np.zeros((ell_.size, t_.size, t_.size))
    for ii in range(t_.size):
        cl[ii] = np.loadtxt(outpath + '/%d.txt'%(ii))
    cl = np.swapaxes(cl, 0, 1)
    np.save(ofolder + '/clphiphi-test', cl)
