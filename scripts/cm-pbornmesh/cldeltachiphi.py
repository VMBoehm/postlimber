...
grid0 = np.zeros((t_.size)).reshape(1, -1)
grid1 = np.zeros((t_.size, t_.size))
...
for index in indexsplit[rank]:
    ...
    for ichi1, chi2max in enumerate(t_*chi1):
        ...
        tosave[:, ichi1] = Cl
        grid1[index, ichi1] = chi2max

    np.savetxt('../output/cldeltachiphi_test/%d.txt'%index, tosave)

    if rank == 0: print('Time taken for index %d = '%index, time()-begin)


grid0 = comm.gather(grid0, root=0)
grid1 = comm.gather(grid1, root=0)
comm.barrier()
...
if rank ==0:
    ...
    print(grid0[0].shape)
    print(indexsplit)
    Grid0 = np.concatenate([grid0[ii][0, indexsplit[ii]] for ii in range(wsize)], axis=-1)
    Grid1 = np.concatenate([grid1[ii][indexsplit[ii], :] for ii in range(wsize)], axis=0)
    print(Grid0.shape, Grid1.shape)
    #np.save('../output/cm_clmesh/cldeltachiphi_chigrid0', Grid0)
    #np.save('../output/cm_clmesh/cldeltachiphi_chigrid1', Grid1)

    cldeltaphi = np.zeros((t_[:50].size, ell_.size, t_.size))
    for i in range(t_[:50].size):
        cldeltaphi[i] = np.loadtxt('../output/cldeltachiphi_test/%d.txt'%i)

    cldeltaphi = np.swapaxes(cldeltaphi, 0, 1)

    np.save('../output/cm_clmesh/cldeltachiphi_test', cldeltaphi)
